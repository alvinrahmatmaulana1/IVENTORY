<template>
  <div>
    ...
    <div class="flex justify-end items-center">
      ...
      <router-link to="/admin/transaksi_keluar/add">
    ...
    <table class="min-w-full border-collapse block md:table">
      ...
      <tbody class="block md:table-row-group">
        ...
        v-for="(transaksi, index) in filteredTransaksi" :key="transaksi.id">
          ...
          <td class="p-2 md:border md:border-white-500 text-center block md:table-cell">
            <span class="inline-block w-1/3 md:hidden font-bold">Tanggal Keluar</span>{{transaksi.tanggal_keluar}}
          </td>
          <td class="p-2 md:border md:border-white-500 text-center block md:table-cell">
          ...
          <td class="p-2 md:border md:border-white-500 text-center block md:table-cell">
            <span class="inline-block w-1/3 md:hidden font-bold">Jumlah Keluar</span>{{ transaksi.jumlah_keluar }}
          </td>
        </tr>
      </tbody>
    </table>
...
export default {
  ...
  computed: {
    ...mapGetters('transaksi_keluars', ['getTransaksi_keluars']),
    filteredTransaksi() {
      return this.getTransaksi_keluars.filter((transaksi) => {
        return transaksi.tanggal_keluar.toLowerCase().includes(this.searchTerm.toLowerCase())
      }).reverse()
    },
  },
  methods: {
    ...mapActions('transaksi_keluars', ['fetchMe']),
    formatRupiah(number) {
